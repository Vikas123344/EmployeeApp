...
export const updateEnquiryApi = async (EnquiryId, formData) => {
  const token = localStorage.getItem('authToken');
  const url = `${BASE_URL}/Enquiry/${EnquiryId}`;
  const options = {
    method: 'PUT',
    headers: {
      'Content-Type': 'application/json',
...
export const viewAllEnquires = async () => {
  const token = localStorage.getItem('authToken');
